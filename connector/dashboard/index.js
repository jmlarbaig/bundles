...
const Floors = nodecg.Replicant('Floors')
...
async function LoadCCData() {
    try {
        let data = { user: '', passwd: '', event: '' }
        data.user = $('#usernameCC').val();
        data.passwd = $('#passwordCC').val();
        data.event = $('#eventId').val();
        console.log(data)
        nodecg.sendMessage('loadCCData', data);
        console.log("Loading CC data")
    } catch (e) {
        console.error("Error loading CC data:", e);
    }
}

Floors.on('change', (newValue, oldValue) => {
    console.log("Floors updated:", newValue);
    if (newValue.length > 0) {
        console.log("Updating floor list with:", newValue);
        $('#floorList').empty();
        newValue.forEach(floor => {
            console.log("Adding floor:", floor);
            $('#floorList').append(`<option value="${floor.id}">${floor.title} (${floor.id})</option>`);
        });
    } else {
        $('#floorList').append('<option value="">No floors available</option>');
    }
})
...
function changeFloor() {
    try {
        if (Connection.value.static == 'connected') {
            Connection()
        }
    } catch {
