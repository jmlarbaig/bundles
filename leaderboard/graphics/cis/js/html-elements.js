...
function createStatsHeader(iDiv) {
    ...
    if (workouts.length > 0) {
        workouts[iDiv].mvt_id.forEach((id, index) => {
            if (index == 0) {
                let $item_header = $(
                    '<th class="col-mvt stats_name" id="header_stats_' + index + '">MVT ' + (index + 1) + '<br><span style="font-size:7px;font-weight:400;color:#252535;">' + (workouts[iDiv].mvt_reps[index] != 0 ? workouts[iDiv].mvt_reps[index] : '') + ' ' + workouts[iDiv].mvt_names[index] + '</span></th>'
                )
