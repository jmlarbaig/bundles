...
function treatFinishStatus(result) {
...
function displayMvtForAthlete(elementAth, isFirstInMvt = true) {
    ...
    if (!isFirstInMvt || !setupFlat.showMvt) {
        elementAth.$item.find(".popup").hide();
        return;
    }
...
function refreshMvt(elementAth, athleteBefore) {
    console.log('refreshMvt', elementAth, athleteBefore);
    if (athleteBefore == null) {
        displayMvtForAthlete(elementAth, true);
        return;
    }
    const isFirstInMvt = (elementAth.currentMvt.id != athleteBefore.currentMvt.id || elementAth.currentMvt.rounds != athleteBefore.currentMvt.rounds);
